...
function flipDigitNow(digit, currentVal, updatedVal) {
  var topFlapNum = digit.querySelector("flap-top > n"),
    bottomFlapNum = digit.querySelector("flap-bottom > n");
...
function updateClock() {
  ...
  for (i = 0; i < epoch.length; i += 1) {
    ...
    if (currentVal !== updatedVal) {
      if (i >= epoch.length - 3) {
        flipDigitNow(digits[i], currentVal, updatedVal);
      } else {
        setTimeout(
          flipDigitTo,
          staggerDelay,
...
function setupClock() {
  ...
  for (i = 0; i < epoch.length; i += 1) {
    ...
    document.querySelector("#ticker").appendChild(digit);
